...
#np.linalg.normはデフォルトでユークリッド距離
#updateは学習率1/nでセントロイドを更新
import numpy as np
...
def initialize_centroids(X_data, k):
  ...
  kmeans.fit(X_data)
  print("kmeansオブジェクト", kmeans.cluster_centers_)
...
def clustering_algorithm(X_stream, k, is_correct_fn, metric='euclidean', initial_X_data_for_kmeans_init=None):
  if initial_X_data_for_kmeans_init is None:
    raise ValueError("KMeans++ の初期化には initial_X_data_for_kmeans_init を指定してください。")
